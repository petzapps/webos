...
*
* JavaScript Source Link: http://javascriptsource.com
*************************************************************************/

function Encryptor(){
}

Encryptor.prototype.encrypt = function (str, pwd) {
  if(pwd == null || pwd.length <= 0) {
    console.error("Please enter a password with which to encrypt the message.");
    return null;
  }
  var prand = "";
  ...
  if(mult < 2) {
    console.error("Algorithm cannot find a suitable hash. Please choose a different password. \nPossible considerations are to choose a more complex or longer password.");
    return null;
  }
...
Encryptor.prototype.decrypt = function (str, pwd) {
  if(str == null || str.length < 8) {
    console.error("A salt value could not be extracted from the encrypted message because it's length is too short. The message cannot be decrypted.");
    return;
  }
  if(pwd == null || pwd.length <= 0) {
    console.error("Please enter a password with which to decrypt the message.");
    return;
  }
  var prand = "";
  for(var i=0; i<pwd.length; i++) {
