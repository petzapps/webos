...
function JSONRpcClient()
{
  ...
  this.authToken = arguments[arg_shift + 3];
  this.objectID=0;
  ...
  if (doListMethods)
  {
    ...
    {
      self = this;
      req.onSuccess = function(result, transport)
      {
...
JSONRpcClient._createMethod = function (client,methodName)
{
  ...
  var serverMethodCaller= function()
  {
    ...
    var req = JSONRpcClient._makeRequest(this, methodName, args, this.objectID, onSuccess, onFailure);
    if (!onSuccess)
    {
...
JSONRpcClient.prototype.createObject = function ()
{
  ...
  req = JSONRpcClient._makeRequest(this, constructorName, _args, 0, onSuccess, onFailure);
  if(onSuccess === null)
  {
...
JSONRpcClient._makeRequest = function (client, methodName, args, objectID, onSuccess, onFailure)
{
  var req = {};
  req.client = client;
...
JSONRpcClient._sendRequest = function (client, req)
{
    ...
    var async = ( req.onSuccess ? true : false );
    ...
    var onSuccess = ( async ?
                      function(transport)
                      {
                          try
                          {	
							//console.log("===========>[JSONrpc] success "+JSON.stringify(transport.responseJSON));
							//console.log("===========>[JSONrpc] success ");
                              var unmarshalled = client.unmarshallResponse(transport.responseJSON);
                          }
                          catch (e)
                          {
								//console.log("===========>[JSONrpc] success - but failure while unmarshalling: ");
                              if (req.onFailure)
							  {
                                  req.onFailure(transport);
								  return; //Added by Petro	
							  }
                          }
                          req.onSuccess( unmarshalled, transport );
                      } :
                      function(transport)
                      {
                          success = true;
    ...
    new Ajax.Request( client.serverURL,
    {
        parameters : req.data,
        asynchronous : async,
        onSuccess : onSuccess,
    ...
    if (success)
        return client.unmarshallResponse(transport.responseJSON);
    else
        throw new JSONRpcClient.Exception({ code: response.status, message: response.statusText });
};
